 ...
 */
    ...
    try {
      this.client.command("index", async (ctx: any) => {
        try {
          ...
          const channelId =
            ctx.message.replyToMessage?.forwardFrom?.chat?.id || undefined;
          const userId = String(ctx.message.from?.id);
          //console.log(ctx.message.replyToMessage)
          const channelName =
            ctx.message.replyToMessage?.forwardFrom?.chat.title || undefined;
          ...
          const isAccessible = await this.client.getMessage(
            channelId,
            INDEXmsgId
          );
  ...
  public async listener() {
    this.client.on("callbackQuery:data", async (ctx: any) => {
      ...
      if (callBackData.startsWith("STREAM")) {
        try {
          ...
          try {
            ...
            setTimeout(async () => {
              let url: any = await this.client.getMessage(
                streamWebHook,
                Number(temp.id) + 1
              );

              if (!url.text) {
                url = await this.client.getMessage(
                  streamWebHook,
                  Number(temp.id) + 2
                );
              }
      ...
      if (callBackData.startsWith("POST")) {
        try {
          ...
          if (!imdbDetails.posterImage.url) {
            temp = await this.client.sendPhoto(
              this.postingChannel,
              this.upiImage,
              {
      ...
      if (callBackData.startsWith("freePlan")) {
        try {
          ...
          if (!isIt) {
            await this.mongo.Unlock(
              String(userId),
              this.client,
              undefined,
              true
            );
            const del = await ctx.reply(
      ...
      if (callBackData.startsWith("Series")) {
        try {
          ...
          if (query && msgId) {
            const msgID = await ctx.editMessageText(
              msgId,
      ...
      if (callBackData.startsWith("Quality")) {
        try {
          ...
          const modif = await ctx.editMessageText(
            Number(msgId),
      ...
      if (callBackData.startsWith("page")) {
        try {
          ...
          const del = await this.client.editMessageText(
            data[1],
            Number(data[2]),
            `< b > RESULTS: \n\nCurrent Page: ${
              Number(data[3]) + 1
      ...
      if (callBackData.startsWith("tutorial")) {
        try {
          ...
          const tempPool = await this.paramsGroupPool(
            data[1],
            "tutorial",
            true
          );
      ...
      if (callBackData.startsWith("showPlans")) {
        await ctx.editMessageText(msgId, this.planDescription, {
          replyMarkup: {
            inlineKeyboard: Markup.planPriceReplyMarkup(
    ...
    this.client.on(":newChatMembers", async (ctx: any) => {
      try {
        ...
        let channelName = ctx?.message?.chat?.title
        ...
        if (newMember == this.botDetails.username) {
          ...
          const d = await this.client.sendMessage(
            ctx.message.chat.id,
            this.groupAddCaption(channelName),
            {
              replyMarkup: {
                inlineKeyboard: Markup.newMemberReplyMarkup(
          ...
          const chatDetails = await ctx.getChatAdministrators(
            ctx.message.chat.id
          );
          const ownerdata = chatDetails.filter(
            (d: any) => d.status == "creator"
          );
        ...
        const d = await ctx.reply(
          `<b>Hᴇʟʟᴏ ${userName} 😍, Aɴᴅ Wᴇʟᴄᴏᴍᴇ Tᴏ ${channelName} ❤️</b>\n\n<b>Just Send Any File Name ill SERCH Open Sourced Available files and <u>List YOU</u>.</b>`,
          {
            parseMode: "HTML",
            replyMarkup: {
              inlineKeyboard: Markup.groupJoinerReplyMarkup(
                this.paymentScreenshotId
              ),
            },
          }
  ...
  private async indexRounds(params: indexInterface) {
    try {
      ...
      const messages: any = await this.client.getMessages(
        params.channelId,
        fileArr
      );
      ...
      for (const message of messages) {
        if (
          ...
          try {
            ...
            await this.client.forwardMessage(
              params.channelId,
              this.supportLog,
              message.id
            );
            ...
            if (error.errorMessage == "MESSAGE_ID_INVALID") {
              ...
              const modified = await params.ctx.editMessageText(
                params.msgToModify,
              ...
              if (currentTasks > 0) {
                ...
                return await params.ctx.editMessageText(
                  params.msgToModify,
  ...
  public async commands() {
    ...
    this.client.command("qr", async (ctx: any) => {
      try {
        ...
        const change = await this.mongo.editBotModel(
          this.botToken,
          fileId,
          "qrFileId"
        );
    ...
    this.client.command("upi", async (ctx: any) => {
      try {
        ...
        if (this.admin.includes(String(ctx.message.from?.id)) && text) {
          ...
          const changeBotModel = await this.mongo.editBotModel(
            this.botToken,
            data[1],
            "upiId"
          );
    ...
    this.client.command("forceSub", async (ctx) => {
      try {
        ...
        if (
          ...
          isReplied &&
          isReplied.forwardFrom?.type == "channel"
        ) {
          const forceSubId = isReplied.forwardFrom.chat.id;

          this.forceSubChatId = String(forceSubId);

          const createInviteLink = await this.client.createInviteLink(
            forceSubId
          );
    ...
    this.client.command("tutorial", async (ctx: any) => {
      try {
        ...
        if (this.admin.includes(String(userId))) {
          ...
          const edited = await this.mongo.editBotModel(
            this.botToken,
            this.tutorialUrl,
            "tutorialVideo"
          );
        ...
        if (chatType == "private") {
          ...
          const groupDetails = await this.paramsGroupPool(
            chatId,
            "userTutorial"
          );
    ...
    this.client.command("set_shortner", async (ctx) => {
      try {
        ...
        if (msgFrom == "group" || "supergroup") {
          ...
          const ownerId = groupDetails.filter(
            (m: any) => m.status == "creator"
          );
          ...
          const isValidUrl = await this.shortenUrlText(
            data[2],
            data[3],
            "www.instagram.com"
          );
          ...
          if (isValidUrl.length < 1) {
            ...
            await ctx.reply("<b>Invalid Url or TOKEN / CONTACT ADMIN !!</b>", {
              replyMarkup: {
                inlineKeyboard: Markup.contactAdminReplyMarkup(
                  this.contactAdmin
                ),
              },
              parseMode: "HTML",
    ...
    this.client.command("set_tutorial", async (ctx: any) => {
      try {
        ...
        const fileAvailable =
          ctx.message.replyToMessage?.video?.fileId || undefined;
    ...
    this.client.command("myPlan", async (ctx) => {
      try {
        ...
        const isPremiumExpired = await this.mongo.isVerified(
        ...
        if (isPremiumExpired == "notVerified") {
          await ctx.reply(
            `Hey ${
              ctx.message.from?.firstName || "user"
      ...
      try {
          ...
          if (match) {
            ...
            if (isVerified == "ban") {
              await ctx.reply(
                ...
                {
                  replyMarkup: {
                    inlineKeyboard: Markup.bannedReplyMarkup(
                      this.paymentScreenshotId
                    ),
                  },
                }
            ...
            const isPower = await this.paramsGroupPool(
              String(chatId),
              "userPowering",
              true
            );
            ...
            if (!isPower) {
              shortenedUrl = await this.shortenUrlText(
                this.apiUrl,
                this.apiToken,
                endPoint
              );
        ...
        if (vals == "/start") {
          ...
          await ctx.reply(this.startCaption(name), {
            parseMode: "HTML",
            replyMarkup: {
              inlineKeyboard: Markup.introReplyMarkup(
  ...
  private async queryManager(
    ctx: any,
    userId: number,
    Query: { query: string; addOn?: String },
    chatId: number,
    firstName: string,
    chatTitle: string
  ) {
    try {
      ...
      if (file.length > 0) {
        ...
        editedMsg = await ctx.editMessageText(
          editedMsg.id,
